...
import { P } from 'dist/client/_astro/Button.B2g5e3XG'
...
const StepsItems = ({ steps }: Props) => {
  return (
    <ol className={clsx('grid gap-4', `grid-cols-${steps.length} md:block md:space-y-4`)}>
      {steps.map(({ TITLE, DESCRIPTION }, index) => (
        <div key={TITLE} className="space-y-8 group md:flex md:gap-4 md:space-y-0 md:items-stretch">
